...
class SparseInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # from backbones import build_resnet_SPD_backbone
        # self.backbone = build_resnet_SPD_backbone()
    ...
    def forward(self, batched_inputs):
        ...
        # # forward
        # # 可视化现在的输入到backbone的image
        # print("images.tensor input")
        # # 将 images 数组转换为可视化的形式
        # import numpy as np
        # import matplotlib.pyplot as plt
        # # 将 images 数组转换为可视化的形式
        # img = np.transpose(images[0].cpu().detach().numpy(), (1, 2, 0))  # 将通道维度放到最后
        # img = img.astype(np.uint8)  # 转换为整型
        # plt.imshow(img)
        # plt.show()

        features = self.backbone(images.tensor)
        features = self.encoder(features) # 1 256 384 256 H/8
        output = self.decoder(features) # logits[1 100 80]  mask[1 100 768 512] score [1 100 1]
    ...
    def inference(self, output, batched_inputs, max_shape, image_sizes):
        ...
        results = []
        pred_scores = output["pred_logits"].sigmoid() # 1 100 80
        pred_masks = output["pred_masks"].sigmoid() # 1 100 384 256
        pred_objectness = output["pred_scores"].sigmoid() # 1 100 1
        pred_scores = torch.sqrt(pred_scores * pred_objectness) # 1 100 80
        ...
        for _, (scores_per_image, mask_pred_per_image, batched_input, img_shape) in enumerate(zip(
            ...
            ori_shape = (batched_input["height"], batched_input["width"]) # 3 7368 4908
            result = Instances(ori_shape)
            # max/argmax
            scores, labels = scores_per_image.max(dim=-1)
            # cls threshold
            keep = scores > self.cls_threshold
            scores = scores[keep] # 100
            labels = labels[keep] # 100
            mask_pred_per_image = mask_pred_per_image[keep] # 100 384 256
            ...
            h, w = img_shape # 3072 2046
            # rescoring mask using maskness
            scores = rescoring_mask(
                scores, mask_pred_per_image > self.mask_threshold, mask_pred_per_image) # 100
            ...
            mask_pred_per_image = F.interpolate(
                mask_pred_per_image.unsqueeze(1), size=max_shape, mode="bilinear", align_corners=False)[:, :, :h, :w] # 3072 2046
            mask_pred_per_image = F.interpolate(
                mask_pred_per_image, size=ori_shape, mode='bilinear', align_corners=False).squeeze(1) # # 3 7368 4908
