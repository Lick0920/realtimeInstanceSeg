...
class Bottleneck_3d_down(nn.Module):
    ...
    def forward(self, x): # 1 64 4 64 64
        shortcut = x
        print("before bottle3d_down shape",x.shape)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.act1(x)
...
def downsample_avg_3d_channel_layer2_1( 
        in_channels, out_channels, kernel_size, stride=1, dilation=1, first_dilation=None, norm_layer=None):
    norm_layer = nn.BatchNorm3d
    avg_stride = stride if dilation == 1 else 1
    if stride == 1 and dilation == 1:
...
def downsample_avg_3d_channel_layer2_2( 
        in_channels, out_channels, kernel_size, stride=1, dilation=1, first_dilation=None, norm_layer=None):
    norm_layer = nn.BatchNorm3d
    avg_stride = stride if dilation == 1 else 1
    if stride == 1 and dilation == 1:
...
def make_blocks(
    ...
    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        ...
        if stride != 1 or inplanes != planes * block_fn.expansion:
            ...
            if stage_idx == 0:
                # 3d downsample
                downsample = downsample_avg_3d_putong(
                    **down_kwargs) if avg_down else downsample_conv_3d_putong(**down_kwargs)
        ...
        for block_idx in range(num_blocks):
            ...
            if stage_name == "layer1":
                ...
                elif block_idx == 1:  # 不down了
                    downsample = downsample_avg_3d_channel_layer2_1(
                    **down_3d_kwargs) if avg_down else downsample_conv_3d_channel(**down_3d_kwargs)
                elif block_idx == 2:  # 
                    downsample = downsample_avg_3d_channel_layer2_2(
                    **down_3d_kwargs) if avg_down else downsample_conv_3d_channel(**down_3d_kwargs) # 
                elif block_idx == 3:  # 怎么改  不down了 ---bottle3 改成普通 boottle3d
                    block_fn = Bottleneck_3d
                    downsample = None
                    # downsample = downsample_avg_3d_putong(**down_kwargs) if avg_down else downsample_conv_3d(**down_kwargs)
                else:                                                              
                    print("error")
            else:
                downsample = downsample if block_idx == 0 else None
...
def build_resnet_vd3d_down23TT_backbone(cfg, input_shape):
...
if __name__ == '__main__':
    ...
    args.config_file = 'D:\project_python\SparseInst/configs\\3d_test\\3d_23TT_houjiangcaiyang.yaml'
