...
class Bottleneck_3d_down(nn.Module):
    ...
    def forward(self, x): # 1 64 4 64 64
        ...
        input_shape_reshaped_list = x.clone().view_as(torch.zeros(input_shape[0], input_shape[1], 2*2, input_shape[-2]//2,input_shape[-1]//2))
        for i in range(2):
            for j in range(2):
                input_shape_reshaped_list[:, :, i*2+j, :, :] = x[:, :, 0,i::2, j::2]
        ...
        shortcut = input_shape_reshaped_list
...
class ResNet(Backbone):
    def __init__(self, block_types, layers, in_chans=3,
        ...
        out_features_names = ["conv1_spd","res2", "res3", "res4", "res5"]
        self._out_feature_strides = dict(zip(out_features_names, [2, 4, 8, 16, 32]))
        self._out_feature_channels = dict(
            zip(out_features_names, [x * BLOCK_TYPE[block_types[0]].expansion for x in [2, 64, 128, 256, 512]]))
        
        # 字典添加一个 lck 0412
        self._out_feature_channels["conv1_spd"] = 1024 # /4
        ...
        for i in range(2): # --> 2
            ...
            layer_branchspd = nn.Sequential(
                ...
                nn.Conv2d(4*out_channels, out_channels*4, kernel_size=1, bias=False),
    ...
    def forward(self, x): 
        ...
        outputs = {}
        x_spd = x.clone()
        print("after conv1 x_spd", x_spd.size())
        # for i in range(3):
        #     x_spd = torch.cat([x_spd[..., ::2, ::2], x_spd[..., 1::2, ::2], x_spd[..., ::2, 1::2], x_spd[..., 1::2, 1::2]], 1)
        x_spd = self.spd_branch(x_spd)
        outputs["conv1_spd"] = x_spd
        print("conv1_spd", outputs["conv1_spd"].size())
...
def build_resnet_vd3d_down_3x2x2_backbone(cfg, input_shape):
...
if __name__ == '__main__':
    ...
    args.config_file = 'D:\\project_python\\SparseInst\\configs\\fenzhi\\333d_down_layer1_res1345.yaml'
    ...
    x = torch.randn(1, 3, 3072, 2048)
    out = model(x)
    print(out["conv1_spd"].shape,out['res3'].shape,out['res4'].shape,out['res5'].shape)
    from mmcv.cnn.utils import get_model_complexity_info
    flops, params = get_model_complexity_info(model, (3, 3072, 2048))
    # 3 2048 2048 --->  122805.22 MFLOPs
    # 3 512 512 ----> 7675.33 MFLOPs 
    split_line = '=' * 30
    print(f'{split_line}\nInput shape: {(3, 3072, 2048)}\n'  # 172.06 GFLOPs
          f'Flops: {flops}\nParams: {params}\n{split_line}')
