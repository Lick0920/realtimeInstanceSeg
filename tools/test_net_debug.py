...
def process_batched_inputs(batched_inputs):
    ...
    # 保持ori_size
    images_shape = images.tensor.shape
    ori_size = (images_shape[2], images_shape[3])
...
class SparseInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        output_shape = self.backbone.output_shape() # res 2 3 4 5 [256 512 1024 2056]
    ...
    def forward(self, image, resized_size, ori_size):
        ...
        features = self.backbone(image) # res3 [1 512 72 128] res4 [1 1024 36 54] res5 [1 2048 18 27]
        features = self.encoder(features) # 1 256 72 128
...
def test_sparseinst_speed(cfg, fp16=False):
    ...
    output_folder = os.path.join("D:\project_python\SparseInst/" + cfg.OUTPUT_DIR, "inference")
...
def setup(args):
    ...
    cfg.merge_from_file('D:\\project_python\\SparseInst\\output_fenzhi\\2d_base_spd1res345_small_bs8(384,600)\\config.yaml')
